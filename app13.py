...
def upload_stock_monthly_valuations_from_excel(uploaded_file):
    """
    Upload a single precomputed stock valuations Excel workbook (.xlsx)
    into fundlab.stock_monthly_valuations using a single UNNEST insert.

    Assumptions:
    - Table fundlab.stock_monthly_valuations exists with columns:
        isin text,
        month_end date,
        ttm_sales numeric,
        ttm_pat numeric,
        ...
        pe numeric,
        pb numeric
      (no constraints / indexes needed for this initial load).
    ...
    # Convert NaNs to None for psycopg / numeric[]
    df = df.where(pd.notnull(df), None)

    # ------------------------------------------------------------------
    # 3) Build UNNEST insert
    # ------------------------------------------------------------------
    insert_sql = text(
        """
    ...
    params = {
        "isin": df["isin"].tolist(),
        "month_end": df["month_end"].tolist(),
        "ttm_sales": df["ttm_sales"].tolist(),
        "ttm_pat": df["ttm_pat"].tolist(),
        "book_value": df["book_value"].tolist(),
        "ps": df["ps"].tolist(),
        "pe": df["pe"].tolist(),
        "pb": df["pb"].tolist(),
    }
    ...
    # ------------------------------------------------------------------
    # 4) Single-shot insert
    # ------------------------------------------------------------------
    engine = get_engine()
    try:
        with engine.begin() as conn:
            st.write("Uploading to Supabase via UNNEST…")
            conn.execute(insert_sql, params)
    except Exception as e:
        # Surface as much detail as Supabase / SQLAlchemy will allow
    ...
    st.success(f"✅ Uploaded {n_rows:,} rows into fundlab.stock_monthly_valuations.")
