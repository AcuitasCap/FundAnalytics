...
def performance_page():
    ...
    st.header("Rolling Returns")
        ...
        # ------------------------ Performance – NAV-based rolling returns and CAGR ------------------------
    st.header("Performance – NAV-based rolling returns and CAGR")

    # Month + Year pickers (shared across Rolling + Yearly + used as default for P2P end)
    ...
    # ---------- Shared filters + analysis mode in a single form ----------
    with st.form("perf_filters"):
        ...
        with cB2:
            end_year    = st.selectbox("End year    (end-domain)", list(range(min_y, max_y+1)),
                                       index=len(range(min_y, max_y+1))-1, key="rr_end_y")
        ...
        apply_filters = st.form_submit_button("Update")

    if not apply_filters:
        st.info("Adjust filters above and click **Update** to see results.")
        return
    ...
    # --------------------------------------------------------------------------------
    # MODE 1: Rolling returns vs. peer average and benchmarks (3Y & 1Y – focus fund)
    # --------------------------------------------------------------------------------
    if analysis_mode == "Rolling returns vs. peer average and benchmarks":
        ...
        if not window_ok(start_domain, end_domain, 36):
            st.warning("Selected domain is too short for 3Y rolling.")
        else:
            # Build rolling series for 3Y
            df_3y_focus, df_3y_peer, df_3y_bench = make_rolling_focus_peer_bench(
                filtered, funds_selected, focus_fund, bench_ser, start_domain, end_domain, window_months=36
            )

            fig3 = plot_rolling_focus_peer_bench(
                df_3y_focus, df_3y_peer, df_3y_bench,
                focus_label=focus_fund,
                peer_label="Peer avg",
                bench_label=bench_label,
                series_selected=series_selected_3,
                title="3Y Rolling — Focus vs Peer avg vs Benchmark"
            )
            st.plotly_chart(fig3, use_container_width=True)

            # 3Y stats
            st.caption("3Y rolling stats — Focus vs Peer avg vs Benchmark")
            stats3 = rolling_stats_focus_peer_bench(
                df_3y_focus, df_3y_peer, df_3y_bench,
                focus_label=focus_fund,
                peer_label="Peer avg",
                bench_label=bench_label,
            )
            st.dataframe(stats3, use_container_width=True)
        ...
        if not window_ok(start_domain, end_domain, 12):
            st.warning("Selected domain is too short for 1Y rolling.")
        else:
            df_1y_focus, df_1y_peer, df_1y_bench = make_rolling_focus_peer_bench(
                filtered, funds_selected, focus_fund, bench_ser, start_domain, end_domain, window_months=12
            )

            fig1 = plot_rolling_focus_peer_bench(
                df_1y_focus, df_1y_peer, df_1y_bench,
                focus_label=focus_fund,
                peer_label="Peer avg",
                bench_label=bench_label,
                series_selected=series_selected_1,
                title="1Y Rolling — Focus vs Peer avg vs Benchmark"
            )
            st.plotly_chart(fig1, use_container_width=True)

            st.caption("1Y rolling stats — Focus vs Peer avg vs Benchmark")
            stats1 = rolling_stats_focus_peer_bench(
                df_1y_focus, df_1y_peer, df_1y_bench,
                focus_label=focus_fund,
                peer_label="Peer avg",
                bench_label=bench_label,
    ...
    # -------------------------------------------------------------------------
        ...
        st.subheader("3Y Rolling — Multiple funds")

        if not window_ok(start_domain, end_domain, 36):
            st.warning("Selected domain is too short for 3Y rolling.")
        else:
            df_3y_multi = make_multi_fund_rolling_df(
                filtered, funds_selected, start_domain, end_domain, window_months=36
            )
            fig3m = plot_multi_fund_rolling(df_3y_multi, "3Y Rolling — Multiple funds")
            st.plotly_chart(fig3m, use_container_width=True)

        st.subheader("1Y Rolling — Multiple funds")

        if not window_ok(start_domain, end_domain, 12):
            st.warning("Selected domain is too short for 1Y rolling.")
        else:
            df_1y_multi = make_multi_fund_rolling_df(
                filtered, funds_selected, start_domain, end_domain, window_months=12
            )
            fig1m = plot_multi_fund_rolling(df_1y_multi, "1Y Rolling — Multiple funds")
            st.plotly_chart(fig1m, use_container_width=True)

    # -------------------------------------------------------------------------
    # MODE 3: Returns (Strict FY/CY endpoints) – uses shared start/end domain
    # -------------------------------------------------------------------------
    elif analysis_mode == "Returns (Strict FY/CY endpoints)":
        st.header("Returns (Strict FY/CY endpoints)")

        # FY / CY selector
        mode_yr = st.radio(
            "Mode",
            options=["Financial Year (FY)", "Calendar Year (CY)"],
            index=0,
            horizontal=True,
            key="yr_mode",
        )

        # This block is your existing yearly returns logic, driven by start_domain/end_domain
        # (you already refactored it earlier to use start_domain/end_domain instead of its own pickers).

        # ---- existing yearly returns code BELOW (unchanged except indentation) ----
        # Everything from:
        #   "st.subheader("Actual yearly returns — Funds (rows) vs Years (columns)")"
        # down to "p_rel = st.checkbox("To print", key="print_rel_tbl")"
        # just keep as-is inside this branch.
        # ---------------------------------------------------------------
        # Paste your yearly-returns block here with one extra indent level
        # ---------------------------------------------------------------

        # Example sketch (replace with your actual block):
        # st.subheader("Actual yearly returns — Funds (rows) vs Years (columns)")
        # ... existing yearly tables, benchmark yearly table, relative yearly table ...
        # p_act = st.checkbox("To print", key="print_act_tbl")
        # p_bmk = st.checkbox("To print", key="print_bmk_tbl")
        # p_rel = st.checkbox("To print", key="print_rel_tbl")

    # -------------------------------------------------------------------------
    # MODE 4: P2P returns + relative vs benchmark (1/3/5/7Y)
    # -------------------------------------------------------------------------
    else:  # "Point to point returns & relative return vs. benchmark (1/3/5/7 year CAGR)"
        ...
        with cP4: p2p_end_y   = st.selectbox("End year", list(range(p2p_min_y, p2p_max_y+1)),
                                             index=len(range(p2p_min_y, p2p_max_y+1))-1, key="p2p_ey")
        ...
        if p2p_end <= p2p_start:
            ...
            if bench_ser is not None and not bench_ser.empty:
                bench_cagrs = {
                    lbl: end_aligned_cagr(bench_ser, p2p_end, m)
                ...
                def rel_colors(dfin: pd.DataFrame):
                    ...
                    for c in dfin.columns:
                        styles[c] = dfin[c].apply(
                            lambda v: "background-color:#e6f4ea;color:#0b8043" if pd.notna(v) and v > 0
            ...
            st.dataframe(style_rel_mh(rel_mh_disp), use_container_width=True)
            p_relmh = st.checkbox("To print", key="print_relmh_tbl")
